...
- Anchor with a concrete number: GPT-3 in 2020 vs. a frontier model in 2025 is already four orders of magnitude of training compute.
- The job of today: teach your internal extrapolator to stop being linear.
> Look at this slide. Two pictures, same prompt. On the left, an astronaut on horseback in a tunnel, the horse's hooves splashing water, the astronaut waving a French flag. It is generated by Stable Diffusion 1, August 2022. The horse has five legs.
>
...
>
> The water splashes correctly under the hooves. The tunnel has perspective. The flag is legible and it is French. Two years apart. Same prompt. This is Figure 11 in the book, and I want you to stare at it for a moment, because staring at it is what this whole session is about.
>
> Today's question is simple. How far does this go? How much room is left in this curve before it hits a wall? Do we have six months of improvement left, six years, sixty years? What would have to go wrong for the curve to stop? And what is the honest answer a physicist would give to a room of undergraduates at Sciences Po in April 2026?
>
> I am going to tell you the answer up front, because I do not like lectures that hide the ball. The honest answer, as best as I can reconstruct it with the public evidence we have, is: there is no visible ceiling before 2030. We know what the rocket is made of. We know what fuels it needs. We know what the supply chain looks like. And in every one of those dimensions, we have enough runway to keep scaling for at least four or five more orders of magnitude of training compute. That is the headline. The rest of the session is the argument.
>
...
>
> Getting used to things that get exponentially faster is crazy hard for human brains. We are evolved for a savannah where change happens on the scale of seasons. We are not evolved for a world where compute doubles every six months. If we want to play a part in what comes next, we have to train that underestimation out of ourselves, and that is uncomfortable, but it is what the job requires.
>
> Second remark. Where there is genuine uncertainty, I will name it. I do not know what happens after ten to the twenty-eighth FLOPs of training. I do not know whether scaling laws hold for another three orders of magnitude or another five. I do not know whether Taiwan will be a factor in 2029. These are real unknowns, and I will flag them as we go. But the hedging has to be specific. The hedging is about named variables, not about mood. I will not say "who knows, it could all stop tomorrow" because, on the public evidence, that is not true.
>
> One more thing before we start. Most of this session uses math. Not hard math. Arithmetic, mostly. A few exponents. I will define every number before I use it. If a number flies past and you have not caught it, raise a hand. It is much better for everyone if we stop for ten seconds than if five of you quietly stop following. Deal?
>
...
> So, scaling laws. Let us start by defining an order of magnitude, because I will use that phrase about two hundred times today and I want it to mean something precise in your head. An order of magnitude is a factor of ten. One more OOM means ten times more. Two OOMs means a hundred times more.
>
> Three OOMs means a thousand times more. Four OOMs, ten thousand. It is the logarithmic unit, the way scientists compare very different sizes. An ant and an elephant differ by about six or seven orders of magnitude in mass. The sun and the earth differ by about six orders of magnitude in diameter.
>
> A GPT-2 model and a GPT-4 model differ by about five orders of magnitude in training compute. When I say "one notch of intelligence per OOM", I mean: each time you multiply compute by ten, you get one more step of capability. That is the scaling laws hypothesis in one sentence. Write it down. It is the single most important empirical observation of the last decade in AI.
>
...
>
> You plot compute on the x-axis in log scale, loss on the y-axis in log scale, and you get a straight line going down. Straight. For four orders of magnitude. No kink, no bend, no saturation visible.
>
...
>
> They are a pattern we observe in the data. We have no fundamental theory for why neural networks scale the way they do. Which means, in principle, they could stop holding tomorrow. We have simply never seen them stop, across now about nine orders of magnitude of exploration. Nine OOMs.
>
> That is a lot of data. Nine OOMs is the difference between a grain of sand and the Empire State Building. So we have good empirical reasons to trust the pattern, but it is not a theorem. It is a regularity. A stubbornly persistent regularity.
>
> Let me give you the numbers so you can calibrate. GPT-3, released in 2020, was trained with roughly three times ten to the twenty-third floating-point operations. Three times ten to the twenty-third. Write the number out: three followed by twenty-three zeros. That is a three and twenty-three zeros of floating-point multiplications and additions chained together during the training run. At the time, it was the largest training run ever. It took a few thousand GPUs, a few months, tens of millions of dollars. In 2020, it felt titanic.
>
> GPT-4 came out in 2023. Public estimates place its training compute at roughly two times ten to the twenty-fifth FLOPs. Do the math. Ten to the twenty-fifth over ten to the twenty-third is a factor of one hundred. Two orders of magnitude. In three years. The model was not just bigger; it was two notches up the scaling ladder. And if you remember your Session 1, you know what two notches looks like: GPT-3 could barely hold a paragraph of coherent French. GPT-4 passes the American bar exam in the top thirty-eight percent of human candidates. Two notches. That is what an OOM looks like in practice.
>
> Let us keep going. OpenAI's o1 model, released in 2024, is estimated at roughly ten to the twenty-seventh FLOPs for training plus inference-time reasoning. Another two orders of magnitude above GPT-4. Then the models coming in late 2025 and 2026, trained inside the Stargate build-out, inside Anthropic's Project Rainier, inside Google's TPU clusters, are projected at ten to the twenty-eighth to maybe ten to the twenty-ninth FLOPs by 2027. That is another two to three orders of magnitude up from o1.
>
> Let me put that trajectory on a timeline so you can feel it. 2020, three times ten to the twenty-third. 2023, two times ten to the twenty-fifth. 2024, ten to the twenty-seventh. 2026, ten to the twenty-eighth. 2027, ten to the twenty-ninth. In seven years, six orders of magnitude. Six OOMs. A factor of one million in training compute. That is the curve. That is the rocket.
>
...
>
> GPT-2 could barely write a grammatical sentence; that was 2019. GPT-4 can pass most standardized exams; that is five OOMs up from GPT-2. The gap between GPT-2 and GPT-4, which is the gap between incoherence and professional competence, is five OOMs. By his reasoning, another four or five OOMs above GPT-4 would take us to something he calls *drop-in remote worker*, a system that can do, autonomously, most of what a junior employee at a large firm does, over the course of weeks of work. And four or five more OOMs above that starts getting us into territory where the model might be meaningfully superhuman.
>
...
>
> I take this criticism seriously. It is intellectually honest and it is backed by data. Where I come down on it is this: even if the mapping from loss to capability is noisier than the scaling laws advocates suggest, the empirical record of the last seven years is that every OOM of compute has, in practice, produced a capability jump that was noticeable and often dramatic. Sometimes it was in one axis, sometimes another.
>
...
>
> We have not, yet, had an OOM that produced no capability increase. That is nine empirical data points, with nine confirmations. At some point, nine confirmations is strong evidence, even if we do not have a theorem. Until we see a flat OOM, it is reasonable to plan as if the pattern holds.
>
> Specifically, the place where I do agree with Hooker is that we do not yet know what happens after ten to the twenty-eighth FLOPs. Maybe the curve bends. Maybe it does not. We literally have not yet trained a model at that scale, though we will in the next twelve to eighteen months. So there is a live empirical question sitting just over the horizon. It is one of the most interesting scientific questions currently being asked.
>
> Let me close this section with a concrete exercise so this does not stay abstract. Suppose one OOM of compute takes about eighteen months to put in place, counting datacenter build-out, chip production, training time. That is roughly the historical cadence over the last five years. Four more OOMs at that rate equals six years of wall-clock time. So if you start the clock today, April 2026, four OOMs takes us to roughly 2032. If the exponential holds for four more OOMs, by 2032 we have models that are four more notches above today's best. Whatever that looks like, it is a lot. That is the math. Eighteen months per OOM, four OOMs, six years.
>
> And here is the subtler point that people miss. The investments are not linearly funded. They are exponentially funded. The money going into AI roughly tripled every year from 2022 to 2025. That means the OOMs are arriving faster, not slower. The eighteen-month cadence is the long-run average. In the current four-year window, it is closer to twelve months per OOM, because the money is flooding in. So four OOMs at the current pace is more like four years, not six. Pick your assumption. Either way, before 2030, we have reasonable grounds to expect the rocket to be several notches above where it is today.
>
> One more thing, because Figure 12 in the book deserves a mention here. Figure 12 plots the difference between the best AI model and the human reference level across a dozen benchmarks, from reading comprehension to mathematical problem-solving to code generation. The striking thing about the figure is the timing.
>
...
>
> Let me start with the datacenter itself. A modern training datacenter is not a building, it is a small city. A frontier cluster today has on the order of a hundred thousand GPUs.
>
> Each GPU is about the size of a shoebox, weighs several kilograms, costs between thirty and forty thousand dollars. One hundred thousand of them, at forty thousand dollars each, is four billion dollars of silicon sitting on racks. Those racks are arranged in rows.
>
...
>
> The next generation of cluster, the one coming online this year and next, is five to ten times bigger. That is where things get interesting, because at that scale you cannot easily site the datacenter in one place anymore. You start to run into local grid limits.
>
> In the US, for example, Virginia is already saturated; Texas has room; parts of the Pacific Northwest have hydro capacity. Microsoft signed a deal in 2024 to restart the Three Mile Island nuclear plant, yes, the same plant that had the famous 1979 accident, to supply dedicated power to a new AI cluster in Pennsylvania. That deal alone is eight hundred and thirty-five megawatts of dedicated nuclear power, going straight to a datacenter. Just for AI training.
>
> The Stargate announcement, January 2025, is the biggest publicly committed infrastructure project in this space. OpenAI, together with SoftBank and Oracle, committed five hundred billion dollars over four years to build a distributed training infrastructure. Five hundred billion. To give you a sense of scale, that is roughly half a percent of US GDP, committed, by three private companies, to one technology category. The first site, in Abilene, Texas, is coming online in pieces through 2025 and 2026. It will pull five gigawatts at completion. Five gigawatts is the output of a large nuclear plant, Gravelines in northern France for instance, running at full tilt.
>
> Anthropic announced its own build-out, called Project Rainier, in late 2024, in partnership with Amazon. That cluster, in Indiana, is planned for about eight hundred thousand Trainium chips, Amazon's in-house AI accelerator, drawing on the order of a gigawatt. Google has been building TPU clusters across several US states and in Finland. xAI, Elon Musk's outfit, built Colossus in Memphis, Tennessee, a single-site cluster of two hundred thousand GPUs brought online in under a year. Meta is building a two-gigawatt cluster in Louisiana called Hyperion. Every big lab has its cluster project. Every cluster pulls hundreds of megawatts to gigawatts. These are not rumors; these are press releases. I will put the links in the slide pack.
>
> To ease the grid pressure, the industry is starting to decentralize. Decentralization here does not mean a peer-to-peer network of gaming PCs; it means spreading one training run across two or three geographically distant clusters connected by very high-bandwidth fiber. You want the fiber to be able to move model weights between sites in seconds, because during training, different replicas of the model need to stay synchronized. If synchronization takes too long, you lose efficiency. So you need petabyte-per-second interconnects between sites. That is extraordinary engineering. Today it is probably just feasible across a single country. In five years it may be feasible continent-scale.
>
...
>
> They make the extreme ultraviolet lithography machines, the EUV machines, that etch chip patterns onto silicon wafers. These machines are, without exaggeration, the most complex objects humans have ever built. Each one is the size of a bus, weighs about two hundred tons, costs two hundred million dollars, and uses a laser to vaporize tiny droplets of molten tin at a rate of fifty thousand droplets per second, producing light at thirteen and a half nanometers wavelength, which is then reflected through a cascade of mirrors polished to atomic flatness, onto a wafer, to draw patterns at three nanometer resolution.
>
> ASML ships about forty of these machines per year. There is effectively no competitor. If ASML stops shipping, advanced chip production globally slows to a crawl within eighteen months.
>
...
>
> TSMC holds roughly fifty percent of the global semiconductor foundry market and close to ninety percent of the leading-edge node market. When NVIDIA designs a new chip, TSMC fabricates it. When Apple designs a new M-series, TSMC fabricates it.
>
...
>
> Below ASML and TSMC in the chain, we have the chip designers. The one you have all heard of is NVIDIA. Santa Clara, California. NVIDIA designs the GPU architectures, the H100, the H200, the Blackwell line called B100 and B200 and GB200. They hold roughly ninety percent of the AI training chip market. Their market capitalization in 2025 oscillated between two and three trillion dollars, which, for a chip design company, is without precedent in economic history. Intel at its peak was worth five hundred billion. NVIDIA at its peak has been worth six times that.
>
...
>
> So the relay race: ASML in the Netherlands ships a lithography machine to TSMC in Taiwan. TSMC uses it to fabricate a wafer designed by NVIDIA in California. The wafer is cut into individual chips, packaged, tested, and shipped to a Microsoft datacenter in Virginia. Microsoft plugs the chip into a server, connects it to a hundred thousand of its cousins, and sells the compute to OpenAI. OpenAI runs a training job. The model gets better. That is the chain. If any one link breaks, the chain stalls.
>
> Now, Moore's law. You have probably heard of it. Gordon Moore, one of the founders of Intel, observed in 1965 that the number of transistors on a chip was doubling every two years. That doubling has held, remarkably, for sixty years now.
>
> But it has slowed. In the 2000s we were doubling every eighteen months. In the 2010s we were doubling every two to three years. Today we are closer to three years per doubling on pure transistor density.
>
> In OOM terms, that is about zero point one five orders of magnitude per year. Slow, in the context of our rocket. The rocket is climbing at point six OOMs per year of training compute. Moore's law, on its own, gives us one quarter of that.
>
...
>
> The cluster grows faster than the individual chip. Second, software and hardware specialization: we use chips more efficiently per operation. We went from FP32, thirty-two-bit floating-point precision, to FP16, to BF16, to FP8, and now to FP4 on the latest Blackwell hardware.
>
...
>
> Altogether, software and architectural efficiency is contributing roughly zero point four five OOMs per year of effective compute, on top of the raw hardware growth. Add it up: zero point one five from Moore's law on transistor density, zero point two or more from investment scaling of cluster size, zero point four five from software, you are comfortably above one full OOM per year of effective compute. One OOM per year. That is a factor of ten in capability-producing fuel, every year. That is the curve.
>
> One last number for your mental model: who owns this compute? Epoch AI, a research group that tracks the compute footprint of the frontier, estimates that as of late 2025, the United States holds roughly three quarters of global training compute for frontier models. China holds about fifteen percent and rising. Europe holds less than five percent. Let me repeat that because it is uncomfortable: Europe, the continent, with four hundred million people and a combined GDP comparable to China's, holds under five percent of the world's frontier AI compute. We are a rounding error. That is a political fact, and we will return to it when we talk about financing.
>
> Figure 20 in the book, the Epoch AI plot, shows this visually. Training compute for frontier models, in FLOPs on a log scale, versus year, from 2019 to 2025. The points climb in a tight line: GPT-3 at the bottom, GPT-4 higher, Gemini 1.0 higher still, Grok 4 at the top of the plot. The slope is zero point six orders of magnitude per year. That line is the rocket's speed. It has held for five straight years. There is no sign of it bending.
>
> Also, I want to come back to the scaling laws I was showing you earlier, because seeing them applied in practice is striking. Figure 18 in the book shows an image-generation model at three different compute levels. Same prompt: a kangaroo in an orange hoodie and sunglasses, holding a "Welcome Friends" sign in front of the Sydney Opera House. At compute times one, you get a deformed blob.
>
> At times one hundred, you get a recognizable kangaroo but it is strange. At times three thousand, you get a photorealistic image, the kangaroo standing, correctly proportioned, sunglasses on, sign in hand. Same architecture. Same prompt.
>
> Just more compute. Three thousand times more compute. That is what OOMs look like in outputs. Remember this image; it is the single best visual I know for the power of scaling.
>
> One last figure while we are here, because the numbers alone do not capture the feeling of the thing. I want you to imagine the floor of a datacenter. Picture the ground. Picture rows and rows of server racks, each about the height of a refrigerator, each holding eight GPUs, stacked floor to ceiling. The aisle between two rows is about a meter wide, just enough for a person to squeeze through. The air is cold because the cooling is constantly running. There is a low, constant hum, not of fans exactly but of airflow through a thousand vents.
>
> The temperature at ear-height might be sixteen Celsius; above the racks, where the hot air rises, it is closer to forty. You walk down that aisle for what feels like a long time. The racks do not end. You pass a numbered sign: rack row 14. Then row 15. At the far end of the building, you turn left and walk another aisle. Another fifty rows.
>
...
>
> Candidate two: semiconductor production. This is a much more serious bottleneck. Two angles. First angle is raw capacity: can we make enough chips? Reference point, the Llama-3.1-405B training, equivalent to GPT-4o class, took roughly thirty-nine million H100-hours cumulative. If you want to do three more orders of magnitude of compute than that, same GPU, you need a thousand times more H100-hours, which is four point five million H100s running continuously for a year. NVIDIA produced roughly one point five to two million H100s in 2024, tripling their 2023 output. If production triples again in 2025, we cross the needed threshold by the end of 2025. So on production capacity alone, the chain can keep up, with some slack. Barely.
>
> Second angle, and this is the one that matters more, is the physical ceiling on chip fabrication. Every generation we have shrunk the transistor: ninety nanometers in 2004, forty-five in 2008, twenty-two in 2012, fourteen in 2014, seven in 2018, five in 2020, three in 2023. The number is the characteristic feature size, more or less. We are heading toward two nanometers this year and eighteen angstroms, so one point eight nanometers, in 2027. The practical question is: can you keep going?
>
> At some point you hit a quantum-mechanical wall. Silicon atoms are roughly zero point two nanometers apart. Below about one nanometer in feature size, the transistor stops behaving like a classical switch. Electrons quantum-tunnel through the gate. The transistor leaks. It cannot hold a voltage. You cannot compute reliably with it. That barrier is real, it is set by physics, and it puts a floor under how small the feature can be.
>
> Is it binding before 2030? No. The industry roadmap has two nanometers this year, one point eight in 2027, one point four in 2029, one in the early 2030s. At the cadence of one node every two years, which the industry has been hitting, the quantum wall bites around 2032 to 2034. So the quantum barrier is a real thing, and it matters for your children's iPhones, but it does not bite in the window we are concerned with today.
>
> The more acute risk in semiconductors is geopolitical. Taiwan. I mentioned earlier that roughly ninety percent of leading-edge chip production goes through TSMC.
>
> And TSMC is on an island that China considers a renegade province, that it has threatened to reunify, by force if necessary, for the better part of eight decades. If Taiwan is blockaded or invaded, or if there is any kind of serious military disruption in the Taiwan Strait, the frontier of AI compute stops for as long as it takes to bring alternate capacity online. The US and EU are both investing massively through the CHIPS Acts to build alternate fabs in Arizona, Ohio, Germany, Japan.
>
> But a leading-edge fab takes three to five years to come online from groundbreaking. So if Taiwan goes dark tomorrow, the world is in a hard ceiling for at least three years on frontier AI compute. That is a real risk. It is probably the single largest binary risk factor on the whole trajectory.
>
...
>
> Candidate three: electricity. This one is getting very real. Look at the numbers. A GPT-3 training run was about one megawatt of instantaneous power. GPT-4 was about ten megawatts. o1 class, about a hundred megawatts. The projected 2026 frontier model, ten to the twenty-eighth FLOPs, needs roughly one gigawatt, the output of a medium nuclear reactor. The 2027 to 2028 target, ten to the twenty-ninth FLOPs, needs ten gigawatts. That is Figure 21 in the book. Each row: one more OOM of FLOPs, one more OOM of cost, one more OOM of power. The numbers are brutal.
>
> Ten gigawatts is a huge number. France's Gravelines, the largest nuclear plant in France, puts out five point five gigawatts. So a 2028 training run would need almost two Gravelines dedicated entirely to it, running for months. That is not something you can casually arrange.
>
> How does that play out per country? China is building generation capacity at an astonishing rate. They have added roughly a hundred gigawatts a year of installed capacity for the last several years, a mix of coal, solar, wind, nuclear, and hydro.
>
> For them, powering a one or ten gigawatt training cluster is a nothing-burger, a rounding error on their annual addition. The US is harder. The US grid is fragmented, regulated at state level, slow to permit new generation.
>
> Finding ten gigawatts of dedicated, reliable, firm power near fiber connectivity is genuinely difficult. This is why you see deals like Microsoft buying the entire output of Three Mile Island, and Amazon trying to site datacenters directly at a nuclear plant in Pennsylvania, and Meta signing long-term wind contracts across the plains states. The US can do it, but it is tight, and it is going to require either rapid permitting reform or a lot of new nuclear or gas.
>
> Europe is a disaster on this dimension. We have decommissioned nuclear plants in Germany, phased out coal, have no political appetite for new build-outs, and the grid is expensive and fragmented across twenty-seven countries. A European frontier-scale cluster, ten gigawatts, firm power, running for a year, is politically and logistically very hard to assemble today. That is why most frontier training is happening in the US and, increasingly, in places like Saudi Arabia and the UAE, where there is willing capital, willing siting, and the ability to build fast.
>
...
>
> Candidate four: financing. How many dollars does this take? Figure 21 again. GPT-3, ten million dollars. GPT-4, a hundred million. o1, about a billion. Ten to the twenty-eighth target, ten billion. Ten to the twenty-ninth target, a hundred billion. A hundred billion dollars for one training run by 2027 or 2028. Let that sink in.
>
> That is an astonishing number. For context, the Manhattan Project cost, in 2024 dollars, about thirty billion. The Apollo Program cost about two hundred and fifty billion. A single AI training run, by 2028, could cost more than the Manhattan Project. And this is not a theoretical number; it is what Stargate's first-phase budget of five hundred billion dollars is setting up to fund. Five hundred billion, over four years, divided across several training cycles and inference infrastructure. It maps onto roughly the right order of magnitude.
>
> Where does this money come from? In the US, it comes from the public markets and from the hyperscaler balance sheets. Microsoft, Amazon, Google, Meta, each of these companies generates annual free cash flow in the range of forty to eighty billion dollars. They can redirect a large fraction of that cash flow into AI infrastructure every year and still pay dividends. SoftBank brings in equity and debt financing. Private equity from the Gulf states adds more. The US capital stack is uniquely well suited to deploying capital at this scale at this speed.
>
> China has different mechanisms: state-directed investment, provincial industrial policy, a lot of it opaque to outside observers. But they are spending, heavily. Whether they are spending three hundred billion a year or eighty billion a year is genuinely hard to know from outside.
>
> Europe? We are the funder of last resort. To give you a reference point, France's 1973 nuclear equipment plan, announced in response to the oil shock, cost thirteen billion francs, which at the time was six percent of GDP.
>
> The equivalent today, at the same fraction of GDP, would be about a hundred and seventy billion euros. If Europe wanted to, we could stand up a frontier-scale AI infrastructure. We have the money, collectively.
>
...
>
> Here is the arithmetic. Llama-3.1 was trained on roughly fifteen trillion tokens. Common Crawl, the largest public web archive, contains something like a hundred trillion tokens of raw text. After deduplication and cleaning, maybe thirty trillion of usable text remains, in datasets like RedPajama-v2. So we have maybe two more OOMs of pure human-written web text, and then we have exhausted the low-hanging fruit. Some analysts, notably the team at Epoch AI, have argued this wall hits around 2027 if we keep scaling data proportionally to compute.
>
> But, but, but. Three things change the picture.
>
> First, text is not the only data. Modern models are multimodal. They train on images, audio, video. YouTube uploads more hours of video in a day than have been transcribed in text in the history of the written word. Yann LeCun has argued, correctly in my view, that a four-year-old human has processed more information through its optic nerve than is contained in the entirety of LLM text training corpora. Once we have the techniques to train on video at scale, the data ceiling moves up by several orders of magnitude. We have not solved video training yet. But we are working on it. The data ceiling moves.
>
> Second, synthetic data. Models can generate their own training data. This sounds paradoxical, like pulling yourself up by your bootstraps. And there are real limits, and there are real collapses that can happen if you do it naively. But, empirically, careful synthetic data pipelines have been producing significant capability gains in the last two years. Anthropic, OpenAI, DeepMind all publicly acknowledge synthetic data plays a major role in their post-training pipelines. Reasoning models like o1 and o3 and o4 are trained substantially on synthetic reasoning traces generated by earlier models. Synthetic data is not a magical bypass, but it is a working tool that extends the data horizon.
>
> Third, better use of existing data. The current training procedure is extraordinarily wasteful. You stream through your fifteen trillion tokens once and call it a day. Students who learn that way flunk. Effective learning involves repetition, reflection, spaced retrieval. Research is showing that running through the same data multiple times with smarter scheduling can extract several extra OOMs of useful signal before you stagnate. Some recent papers suggest you can go up to about forty passes before returns really diminish. That is more than one full OOM of effective data from the same corpus, for free.
>
> Add those three together: multimodal expansion, synthetic data, better training procedures. The naive "we are running out of tokens" story underestimates by at least two or three OOMs. Before 2030, data is not going to be the binding constraint either.
>
...
>
> Figure 22 in the book, I want to mention it here, lays out this whole story in three side-by-side bar charts. The left bar chart is electricity: GPT-4 at ten megawatts, Stargate cluster today at a few gigawatts, and the ten-to-the-twenty-eighth target at around one gigawatt for one training. The middle chart is funding, comparing GPT-4 at a hundred million, current state around ten billion, target at one hundred billion.
>
...
>
> No one dimension is three OOMs behind another. That is the remarkable fact, and it is why the rocket can actually get built. You do not have a situation where you have the energy but no money, or the chips but no data. You have, currently, all three lining up. That will not always be true, but it is true now.
...
- Inference cost has fallen by roughly two OOMs in two years; once a model can do a task, it runs cheap.
...
>
> A single ChatGPT query, averaged over the distribution of queries the service actually sees, consumes on the order of zero point three to three watt-hours of electricity. Call it a watt-hour, order of magnitude. That is less energy than a forty-watt incandescent lightbulb running for about a minute and a half.
>
> That is it. A full Google search, end to end, consumes about zero point three watt-hours as well. ChatGPT is at most a few times more energy-intensive than a Google search.
>
...
>
> Let me walk through the numbers another way, because this is the number that changed my mind about economic impact. A mid-size frontier model, say the GPT-4o-mini class, can answer roughly a million tokens of high-quality output for fifteen cents, at OpenAI's published 2024 API price. A million tokens is about seven hundred and fifty thousand words. Fifteen cents. A professional writer, at French minimum wage, would take about ten thousand hours to write seven hundred and fifty thousand words; that is roughly a hundred thousand euros in labor cost. The ratio is six orders of magnitude. Six OOMs cheaper than a human writer.
>
> Now of course that comparison is not quite fair. The model does not write as well as a professional human writer for literary fiction. For boilerplate text, legal summaries, translation, first-pass code, the model is already competitive or better. So the fair comparison is: for tasks where the model is competent, it is one hundred to one hundred thousand times cheaper than a human. Pick your task. The lower bound of the range is already a factor of a hundred. On tasks the model is genuinely good at, it is cheaper by a factor of ten thousand or more.
>
> And here is the trend: inference costs have been falling like a stone. At constant capability, the cost to serve a model has fallen by roughly a factor of a hundred in the last two years. Sam Altman pointed this out in 2024 on his Twitter: text-davinci-003, OpenAI's November 2022 model, cost twenty dollars per million tokens of output. GPT-4o-mini, released in mid 2024, is actually more capable than text-davinci-003 on every benchmark, and it costs fifteen cents per million tokens. Twenty dollars down to fifteen cents. More than a factor of a hundred, in less than two years. Two OOMs of inference cost reduction at constant capability.
>
...
>
> Take the concrete example of the GAIA benchmark we will talk about next week when we cover agents. GAIA gives agents real-world tasks like "find me the cheapest flight from Paris to New York next session with a morning departure." Solving one GAIA task with a current agentic system costs about one euro per question. A human annotator, at French minimum wage, costs roughly three or four euros to solve the same task in the time it takes them. So the agent is already cheaper than minimum wage, and this ratio is going to widen fast as inference prices fall another one or two OOMs over the next two or three years.
>
> Think about what that means, economically. You have a worker that costs less per task than a minimum-wage human, works twenty-four hours a day, does not require benefits, vacation, healthcare, or training, produces output in seconds rather than hours, and can be instantiated a million times in parallel from one training run. That is not a curiosity.
>
...
>
> You just need it to be adequate, at a tenth the cost, at a thousand times the speed. The economics blow up the second those two conditions are both met. And for a growing share of tasks, they are already both met.
>
...
>
> In San Francisco alone, Waymo now has around twenty percent of the ride-hail market and is growing fast at the expense of Uber and Lyft. The price per ride is competitive because there is no human driver to pay. If you had told me in 2018 that by 2025 there would be a commercially profitable robotaxi service operating in a major US city, I would have given you twenty-to-one odds against. And it is here. That is autonomous driving, arguably the hardest physical-world robotics problem other than general-purpose dexterity, solved, in production.
>
> Humanoid robots next. Unitree, a Chinese company based in Hangzhou, sells the G1 humanoid robot for thirty thousand US dollars. Thirty thousand. That is the price of a mid-range new car in France.
>
> The G1 walks, balances, picks up objects, dances, does martial arts demonstrations. You can see the videos online, they are worth your ten minutes. Boston Dynamics has the Atlas platform; Figure AI has Figure 02; 1X has Neo; Tesla has Optimus. All of these are humanoid robots, and all of them are accelerating toward commercial deployment in the next eighteen to thirty-six months.
>
> The hardware is here. The mechatronics, the sensors, the actuators, the batteries, all of it, is here. What was holding humanoid robots back for twenty years was the software, specifically the question of how to generalize motor control across unseen situations. Transformer models plus reinforcement learning plus massive teleoperation data are solving that. Clumsily at first, then increasingly well.
>
> So the "AI only matters for digital work" argument is already wrong and will be obviously wrong by 2027. Robots plus agents plus frontier intelligence form a stack that reaches into the physical world. Your grandfather's image of a robot, the stiff humanoid of 1960s sci-fi, is becoming real. And it is cheap. Thirty thousand dollars for a humanoid robot is already in the range where a small business can justify buying one if it can replace a part-time worker. And these prices will fall.
>
> One more deployment point. Inference is also becoming physically portable. Frontier-class models no longer need to run in a datacenter. A quantized version of Llama-3, or Mistral, or a distilled version of Claude, runs on a laptop today. On an iPhone 16 Pro, you can run a seven-billion-parameter model with acceptable latency. This means the AI you interact with does not always need to go over the network. It can live on your device. That is relevant for privacy, for latency, for offline use, and for sovereignty. A Europe that invests a little in inference-side tooling can actually carve out a real presence here, even without owning the training frontier.
>
...
>
> Let me linger on one specific number because it is the one I find most clarifying. A forty-watt bulb. You know what a forty-watt bulb looks like, it is the soft yellow bedside lamp in every French grandmother's flat. If you leave a forty-watt bulb on for ninety seconds, that is roughly the energy of one ChatGPT query. Ninety seconds of a bedside lamp.
>
> The entire cost of the query is the inference compute, the electricity to the datacenter, the share of the cooling, the server maintenance. Ninety seconds of a bedside lamp. Now scale that up. A hundred ChatGPT queries in a day is fifteen minutes of the bedside lamp. A thousand queries is two and a half hours of the lamp.
>
...
>
> One more angle on deployment that does not get enough attention: the speed of inference. A ChatGPT response that takes you thirty seconds of work to read through took the model perhaps two seconds to produce. The model can in principle produce a thousand such responses in parallel, on a single large GPU cluster, at the same time. One model. Thousand simultaneous conversations. Each one as competent as the model's single-conversation competence.
>
> There is no fatigue. There is no coffee break. There is no distinction between Tuesday morning and Sunday evening. The model answers the thousandth query of the shift as sharply as the first. You imagine what a human workforce would cost to achieve the same throughput, across multiple languages, twenty-four seven, and the comparison is not even close. That is the economic earthquake inbound.
>
...
>
> First place: it plateaus well below human expert level. This is the "LLMs are a dead-end" story. Scaling laws break around ten to the twenty-eighth FLOPs, and beyond that, adding compute stops adding capability. We stay with models that look roughly like 2026-era Claude or GPT, impressive but not transformative. Some prominent researchers hold this view: Yann LeCun, at Meta, has been saying for years that LLMs alone are fundamentally limited, that they cannot achieve true world-modeling without architectural innovations. I respect Yann's view. I think it is wrong on timing, but I think it is defensible on the far horizon.
>
> Second place: it plateaus around top human expert level. The model reaches the competence of a best-in-class professional in every knowledge domain, but does not exceed it. Something like: the best doctor, the best lawyer, the best mathematician, the best coder, accessible to everyone, at a penny a query. That is a very optimistic world for human welfare, and a world that many people find both credible and sufficient. You could call this the "distill-and-democratize" ceiling: the value is not in surpassing the best human but in making the best-human-equivalent ubiquitous and cheap.
>
...
>
> Even if you think I am wrong, even if you think the ceiling is at the second place, that is still a world transformed. A world where everyone has access to Nobel-laureate-level analysis on every question, at marginal cost, is a radically different civilization from the one we grew up in. You do not need superintelligence to have a revolution. You just need the median quality of cognitive work, accessible to everyone, to move up significantly. That alone restructures labor markets, education, healthcare, law, science.
>
...
>
> This is the fourth narcissistic wound. And it is happening in your lifetime. Not in some abstract future; right now. You are living through it. You will remember where you were when it became unmistakable, the way people remember where they were on September eleventh or during the fall of the Berlin Wall.
>
...
> Let me do a recap. We covered a lot of ground, and I want you to leave with the structure clear, because the structure is what you are going to refer back to when you read the news over the next five years.
>
> The rocket. That is the core metaphor. The rocket has three fuels. Energy, measured in gigawatts. Compute, measured in FLOPs. Data, measured in tokens. You need all three. You pull any one of them, the rocket stalls. The engine of the rocket is an algorithm, the Transformer, with some variants that we will meet more of in later sessions. The pilot is the research community, which keeps improving efficiency. The funding comes from capital markets, which are deploying capital at trillion-dollar scale. The trajectory is scaling laws, empirical, durable so far, and what we have seen over nine OOMs of exploration is a near-linear improvement in capability per linear increase in the log of compute.
>
> And we have spent this session answering one question: how far does that rocket go before it runs out of fuel? And the answer is: on every one of the five candidate ceilings, we have argued that the constraint does not bite before 2030. Rare earths, not the bottleneck. Semiconductors, supply keeps up, with Taiwan as the tail risk. Electricity, tight but doable. Capital, abundant where it matters. Data, not running out. Therefore, unless several of these constraints tighten simultaneously in a coordinated way that we do not currently see, the rocket keeps climbing for at least another three to five OOMs of training compute. Six to eight years at the current pace. That takes us well past 2030.
>
> I promised a Mike Powell analogy at the start, so let me deliver it. Mike Powell, American long jumper, set the men's long jump world record in 1991 at eight meters ninety-five. It has stood for thirty-four years. Astonishing.
>
> Now, here is the thing about long jump. A world-class male long jumper jumps eight to nine meters. An amateur club competitor jumps five to six meters. A high school athlete jumps around four.
>
> A child jumps one to two. Literally, on a scale of ability, the entire interesting range of human long jump performance falls inside a window of about four meters, between five and nine meters. Within that range, an extra centimeter is a lifetime of work. Powell's record-setting jump at the 1991 World Championships, eight ninety-five, was centimeters ahead of Beamon's previous record from 1968 that had stood for twenty-three years.
>
> Now imagine a robot that is learning to jump. In the robot's first years, it has to learn to stand. Then to walk. Then to run. Then to jump at all. All of that happens below one meter. For years, the robot jumps zero meters. Then, suddenly, through improvements in balance, control, motor power, it jumps one meter. Then two. Then four. Then five. Five meters is world-amateur. Seven is regional champion. Eight is Olympic medal. Once the robot is at eight, the step from eight to nine is trivial by the standards of everything that came before. Improving the motors another ten percent, or the control software another ten percent, buys you that last meter.
>
> AI is the robot. We spent sixty-five years teaching it to jump at all. We have been stuck at zero for most of that time. Then in 2017, with Transformers, we started jumping.
>
> Then we got to one meter, then two. By 2020, maybe four. By 2022, seven. By 2025, eight and a half, roughly human-expert on many tasks.
>
> The step from eight and a half to nine, which is beyond Powell's record, does not require a new theory. It requires what it has always required: more fuel, more iteration, the same engine running a little longer. The hard part is done. The remaining part is the part we are good at, which is engineering at scale with a lot of money.
>
...
>
> Most of the hard work is behind us. The mirror-image framing, which you will often hear, that "these AIs are not as good as the best humans, so they cannot possibly keep improving," is exactly backwards. They have gone from zero to almost-human in eight years. They are not going to stop at the human line. The human line is an arbitrary biological point on an infinite continuum of possible performance. There is no reason the curve respects that point.
>
...
>
> If this still feels uncomfortable, if you still feel the pull of "surely there is a catch," I want you to name the catch. What exactly? Not a vibe, a specific mechanism. Is it energy? We addressed it. Is it data? We addressed it. Is it chips? We addressed it. Is it Taiwan? A real risk, but a tail risk. Is it capital? It is flooding in. Is it some mysterious asymptote nobody has yet seen in nine OOMs of scaling? Possible, but we would need to actually hit it first. Name the mechanism. If you cannot, then accept that the exponential is your base case and plan accordingly.
...
>
> Today we answered, as tightly as I know how, the question of how far AI will go. The answer, in one sentence: the exponential is real, the fuels are known, and the ceiling is above us. The rocket was built over the last sixty-five years, it took off in 2017 with Transformers, it has been climbing at one OOM of effective compute per year since 2020, it will continue to climb through 2030 unless a specific coordinated failure of energy, chips, data, or capital forces it to stop, and none of those failures is likely on current trends.
>
> By 2028 or 2029, the compute used to train a single model will cost on the order of a hundred billion dollars, draw on the order of ten gigawatts of power, and produce a model several notches above today's frontier. What that looks like is genuinely hard to imagine. I do not know. Nobody knows. What we know is that it will be more capable, by a lot, than what we have today.
>
> In the process of arguing for the exponential, we did not dodge the uncertainties. We named them: Taiwan, European grid capacity, post-ten-to-the-twenty-eighth scaling, post-training data scarcity. These are live questions. We should watch them. They are tail risks, not base-case showstoppers.
>
...
>
> Next week, Session 4. We meet the workers. AI agents. The systems that take the language model we have been studying and give it hands. A tool box. A browser. A file system. A credit card. You will see how the loop works, we will build one together in code, and you will understand why the transition from "chatbot" to "employee" is the most important product transition of the AI era. It is the thing that takes AI from being a curiosity to being an industrial revolution. If today was about "how smart can we make it," next week is about "so what can it actually do for me, right now, today." Come ready. Bring a laptop.
>
> Before you leave, two housekeeping items. First, the reading for next week is the GAIA benchmark paper by Mialon et al. 2023, twelve pages, straightforward. Please read it. I will be assuming you have. Second, if you take away one question from today, let it be this: what concepts in my own life rest on assumptions about human cognition that might stop being true?
>
...
>
> Turn those concepts over, look at the assumptions under them, and ask whether the assumptions still hold. That is the Tomas Pueyo exercise. Do it this week. It is a five-minute exercise. It will save you time later.
>
...
- OpenAI, SoftBank, Oracle, *Stargate Project announcement*, January 2025. Five hundred billion dollars over four years, starting with Abilene, Texas cluster.
- Microsoft and Constellation Energy, *Three Mile Island Unit 1 power purchase agreement*, September 2024. Eight hundred thirty-five megawatts of dedicated nuclear power for Microsoft AI datacenters.
- Anthropic, *Project Rainier* with Amazon Web Services, announced 2024. Large-scale Trainium deployment in Indiana, roughly one gigawatt target.
...
> They might. Nobody has a theorem that says they will not. What we have is nine orders of magnitude of empirical confirmation, from 2017 to 2025, with no kink in the curve yet. Each OOM has produced a visible capability jump. In the absence of a known mechanism that would cause a plateau, and given how much capital is riding on the continuation of the trend, the base case is that scaling laws hold for at least another three to five OOMs. The honest uncertainty is about what happens past ten to the twenty-eighth FLOPs, because we have not yet trained there. We will within twelve to eighteen months. Then we will know.
...
> That is the single biggest tail risk on the 2025 to 2030 trajectory. If TSMC's leading-edge fabs go offline tomorrow, global AI compute production drops to maybe twenty percent of current output for as long as it takes to bring alternate capacity online, which, given the three-to-five-year lead time for a new fab, means a hard ceiling for at least that long. That would delay the rocket by two to four years.
>
...
> The training footprint is real but smaller than most people think, and the inference footprint is much smaller. A ChatGPT query uses roughly one watt-hour, the energy of a forty-watt bulb running for a minute and a half. That is significant at scale, but not, per-use, large compared to, say, streaming a video. The training of a frontier model uses gigawatt-hours for weeks, which maps to roughly the annual electricity consumption of a small town, per model.
>
...
> An order of magnitude, OOM, is a factor of ten. One more OOM means ten times bigger or ten times more. Two OOMs means a hundred times.
>
> Three OOMs, a thousand times. It is the logarithmic step size that scientists use when they compare quantities that span a huge range. If a metric grows by one OOM per year, it is multiplied by ten every year, by a hundred in two years, by a thousand in three years.
>
> The key thing to remember is that OOM growth compounds exponentially in your intuition but linearly on a log scale. When someone says "four more OOMs to AGI," they mean ten thousand times more compute than today. That is a big number, but at one OOM per year, it is four years.

**Q: Are we running out of data to train on?**
> Not before 2030, and possibly never. The naive concern, which some analysts have raised, is that we have roughly thirty trillion tokens of usable public text, and current models train on fifteen trillion. So we have maybe one more OOM of text data before we run out.
>
...
> That is the policy question, and I do not have a comfortable answer. The numbers are what they are: Europe holds under five percent of global frontier AI compute. Building a true frontier-scale European training capacity would take coordinated action across several member states, on the order of a hundred and fifty to two hundred billion euros over five years, comparable to the 1973 French nuclear program as a fraction of GDP. We have the money, collectively.
>
